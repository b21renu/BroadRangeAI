...
def answer_question_with_transformers(transcript, question, target_language):
    print(f"Received question inside python_code.py: {question}, target_language: {target_language}")
    question_language = detect(question)
    max_context_length = 4095 #8191
    # truncated_transcript = ' '.join(entry['text'] for entry in transcript)
    truncated_transcript = transcript[:max_context_length - len(question)]
...
def a_q_w_t_pdf(pdf_summary, question, target_language):
    print(f"Received question inside python_code.py: {question}, target_language: {target_language}")
    question_language = detect(question)
    #max_context_length = 4095 #8191
    # truncated_transcript = ' '.join(entry['text'] for entry in transcript)
...
def a_q_w_t_both(pdf_summary,transcript, question, target_language):
    print(f"Received question inside python_code.py: {question}, target_language: {target_language}")
    question_language = detect(question)
    max_context_length = 4095-200 #8191
    # truncated_transcript = ' '.join(entry['text'] for entry in transcript)
    truncated_transcript = transcript[:max_context_length - len(question)]
