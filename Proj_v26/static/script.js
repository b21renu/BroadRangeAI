...
function askQuestion() {
    //retreiveing the summary content
    const summariesContent = document.getElementById('summaries').innerHTML;
            /*fetch('/store-summaries', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
                body: JSON.stringify({ summariesContent: summariesContent })
            })
            .then(response => response.json())
            .then(data => {
                console.log("Summaries content sent to server:", data);
            })
            .catch(error => {
                console.error("Error sending summaries content:", error);
            });*/
    ...
    fetch('/ask_question', {
        ...
        body: `question=${encodeURIComponent(question)}&target_language=${encodeURIComponent(targetLanguage)}&content_type=${encodeURIComponent(contentType)}&summariesContent=${encodeURIComponent(summariesContent)}`,
    })
    .then(response => response.json())
    .then(result => {
        if (result.transcript) 
